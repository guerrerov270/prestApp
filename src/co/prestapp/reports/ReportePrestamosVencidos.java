...
public class ReportePrestamosVencidos {
	...
	public ReportePrestamosVencidos(String titulo, String nomPDF) {
		strRotuloPDF = titulo;
		...
		try { // Hoja tamanio carta, rotarla (cambiar a horizontal)
			...
			e.printStackTrace();
			error.guardarMensajeError(e.getMessage(), this.getClass().getCanonicalName() + ".ReportePrestamosVencidos");
		}
	}
	...
	private void agregarTabla(Paragraph parrafo) {
		...
		PdfPCell cell = new PdfPCell(new Paragraph("Listado de préstamos vencidos"));
		cell.setColspan(12);
		...
		try {
			...
			CallableStatement miProcedimiento = conexion.prepareCall("{call listar_prestamos_vencidos}");
