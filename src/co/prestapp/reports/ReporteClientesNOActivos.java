...
public class ReporteClientesNOActivos {
	...
	public ReporteClientesNOActivos(String titulo, String nomPDF) {
		strRotuloPDF = titulo;
		...
		try { // Hoja tamanio carta, rotarla (cambiar a horizontal)
			...
			e.printStackTrace();
			error.guardarMensajeError(e.getMessage(), this.getClass().getCanonicalName() + ".ReporteClientesNOActivos");
		}
	}
	...
	private void agregarTabla(Paragraph parrafo) {
		...
		PdfPCell cell = new PdfPCell(new Paragraph("Listado de clientes no activos"));
		cell.setColspan(6);
