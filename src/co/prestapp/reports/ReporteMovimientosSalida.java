...
public class ReporteMovimientosSalida {
	...
	public ReporteMovimientosSalida(String titulo, String nomPDF) {
		strRotuloPDF = titulo;
		...
		try { // Hoja tamanio carta, rotarla (cambiar a horizontal)
			...
			e.printStackTrace();
			error.guardarMensajeError(e.getMessage(), this.getClass().getCanonicalName() + ".ReporteMovimientosSalida");
		}
	}
	...
	private void agregarTabla(Paragraph parrafo) {
		...
		PdfPCell cell = new PdfPCell(new Paragraph("Listado de movimientos"));
		cell.setColspan(11);
